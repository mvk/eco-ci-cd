...
ARG ANSIBLE_GALAXY_INSTALL_OPTS="--collections-path ${ANSIBLE_COLLECTIONS_PATH} --no-cache --force --pre"
ARG PYTHONUNBUFFERED=1
ARG PYTHONDONTWRITEBYTECODE=1
ARG PIP_OPTS="--no-color --no-cache-dir"
ARG PIP_INSTALL_OPTS="--prefer-binary"
...
RUN echo "BEFORE pip install -r requirements-base.txt. Disk Usage:" && du -xsk /
...
RUN python3 -m pip ${PIP_OPTS} \
        install ${PIP_INSTALL_OPTS} --upgrade \
        -r requirements-base.txt

RUN echo "BEFORE pip install -r requirements-container.txt. Disk Usage:" && du -xsk /
# Install ansible and ansible-lint
RUN python3 -m pip ${PIP_OPTS} \
        install ${PIP_INSTALL_OPTS} \
        -r requirements-container.txt

RUN echo "BEFORE ansible-galaxy collection install. Disk Usage:" && du -xsk /
# Install requirements
RUN ansible-galaxy collection \
    install ${ANSIBLE_GALAXY_INSTALL_OPTS} \
    -r requirements.yml

RUN echo "BEFORE dnf clean all. Disk Usage:" && du -xsk /
RUN find . -type d -name .ansible | xargs -I{} echo "{}" && du -sk "{}"
# Clean up the system
RUN dnf clean all

RUN echo "END: Usage. Disk Usage:" && du -xsk /

# Clean up unnecessary files in the same layer
# RUN find /usr /opt /var -name "*.c" -delete 2>/dev/null || true && \
#       find /usr/share/man -type f -delete 2>/dev/null || true && \
#       find /usr/share/doc -type f -delete 2>/dev/null || true && \
#       find /usr /opt /var -name "*.c" -not -path "*/python*" -delete 2>/dev/null || true && \
