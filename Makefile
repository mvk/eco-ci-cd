...
# ansible-builder params
AB_VERBOSITY							?= 2
AB_EXTRA_BUILD_CLI_ARGS		:= $(PODMAN_BUILD_PARAMS) --build-arg-file ../$(BUILD_ARGS_FILE)
...
ifeq ($(SCRIPT_DEBUG),1)
	...
	AB_EXTRA_BUILD_CLI_ARGS += --log-level debug
	AB_VERBOSITY = 3
endif
...
ee-build: image-build-args-file venv-ensure ee-sync-requirements
	...
	@source $(VENV_DIR)/bin/activate && \
		cd $(EE_DIR) && ansible-builder build \
			...
			--extra-build-cli-args="$(AB_EXTRA_BUILD_CLI_ARGS)" \
			--verbosity $(AB_VERBOSITY)
