...
- name: merge files with specific strategy and output to file
  ansible.builtin.set_fact:
    result_path: "{{ ['f1.json', 'f2.json'] | reportsmerger(strategy='normal', output_file='result.json') }}"
```
...
- name: merge and save to file
  ansible.builtin.set_fact:
    output_path: "{{ files | reportsmerger(strategy='large', output_file='/tmp/merged.json') }}"
